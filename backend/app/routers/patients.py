...
def get_patient_timeline(
    ...
    if x_user_role == "patient":
        if not x_patient_uid or x_patient_uid.upper() != uid.upper():
            raise HTTPException(status_code=403, detail="Access denied: You can only access your own timeline.")
...
def get_clinical_contexts(
    ...
    if x_user_role == "patient":
        if not x_patient_uid or x_patient_uid.upper() != uid.upper():
            raise HTTPException(status_code=403, detail="Access denied: You can only access your own clinical contexts.")
...
def get_clinical_brief(
    ...
    if not generate_if_missing:
        if cached:
            print("PATIENT PORTAL FETCH: Returning persisted summary.", flush=True)
            return cached.brief_json
        else:
            raise HTTPException(
                status_code=404,
                detail="The clinical summary for this context has not been generated by the hospital admin yet."
            )
